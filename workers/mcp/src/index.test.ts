...
describe("relationship tools in tools/list", () => {
  ...
  it("total tool count is 37 (25 base + reconnection + briefing + 3 milestone + group_schedule + cognitive_load + context_switches + simulate + caldav_feed + query_graph + risk_scores)", async () => {
    const handler = createMcpHandler();
    const env = createMinimalEnv();
    const authHeader = await makeAuthHeader();
