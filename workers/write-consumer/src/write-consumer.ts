...
export class WriteConsumer {
  ...
  private async handleDelete(msg: DeleteManagedMirrorMessage): Promise<ProcessResult> {
    ...
    try {
      ...
      //
      // Exception: origin_cascade deletes target user-created origin events
      // that don't carry tminus/managed markers. These are trusted because
      // the UserGraphDO verified the canonical exists before enqueuing.
      const isOriginCascade = msg.origin_cascade === true;
      if (!isOriginCascade) {
